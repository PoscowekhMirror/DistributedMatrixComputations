    ...
    where T : INumber<T>
{
    ...
    public IEnumerable<T> GetValuesOnly()
    {
    ...
    // public SparseVector(IEnumerable<Element<T>> elements) : this(elements.Select(e => e.Value)) { }
    public SparseVector(IEnumerable<T> elements)
    {
        var count = -1;
    ...
    public SparseVector(int count, IEnumerable<IndexedElement<T>> elements)
    {
        Count = count;
        _indexedElements = 
            elements
    ...
    internal SparseVector(int count, in IList<IndexedElement<T>> elements)
    {
        Count = count;
